...
We encourage users which are not familiar with MCMC to start with the :doc:`content/installation` page to install the baredSC package. Then, follow the tutorial for a single gene (:doc:`content/tuto/tutorrial_sim_default_1d`) which is a step by step example of application using an input provided on the github repository with default parameters. The other tutorials describe influences of some parameters or use a 2d example.
